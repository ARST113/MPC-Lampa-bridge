...
bool CLampaBridge::Command(CMainFrame* pMainFrame, const CStringA& payloadBody, CStringA& body){ rapidjson::Document d; if(d.Parse(payloadBody).HasParseError()||!d.IsObject()){ body="{\"ok\":false,\"error\":\"invalid json\"}"; return true;} CStringA cmd; getJsonValue(d,"command",cmd); cmd.MakeLower(); if(cmd=="play"){ pMainFrame->SendMessageW(WM_COMMAND,ID_PLAY_PLAY); EmitPlaybackStateChanged(pMainFrame);} else if(cmd=="pause"){ pMainFrame->SendMessageW(WM_COMMAND,ID_PLAY_PAUSE); EmitPlaybackStateChanged(pMainFrame);} else if(cmd=="stop"){ pMainFrame->SendMessageW(WM_COMMAND,ID_PLAY_STOP); EmitPlaybackStateChanged(pMainFrame); EmitSessionFinished(pMainFrame, "user");} else if(cmd=="seek"){ double tm=0; if(d.HasMember("time")&&d["time"].IsNumber()) tm=d["time"].GetDouble(); int64_t from=PositionMs(pMainFrame); pMainFrame->SeekTo((REFERENCE_TIME)(tm*10000000.0)); EmitSeekCompleted(pMainFrame, from, (int64_t)(tm*1000.0)); EmitPositionTick(pMainFrame, true);} else if(cmd=="next"&&m_session.activeIndex+1<(int)m_session.items.size()){ pMainFrame->PostMessageW(WM_COMMAND, ID_NAVIGATE_SKIPFORWARD); m_session.activeIndex++; auto& it=m_session.items[m_session.activeIndex]; m_session.activeUrl=NormalizePlayableUrl(it.url); m_session.activeTitle=it.title; m_session.activeTimelineHash=it.timelineHash; m_session.requestedPosition=it.position; ApplyPendingSeek(pMainFrame); EmitPlaylistItemChanged(pMainFrame, "next"); EmitPlaybackStateChanged(pMainFrame); EmitPositionTick(pMainFrame, true);} else if(cmd=="prev"&&m_session.activeIndex>0){ pMainFrame->PostMessageW(WM_COMMAND, ID_NAVIGATE_SKIPBACK); m_session.activeIndex--; auto& it=m_session.items[m_session.activeIndex]; m_session.activeUrl=NormalizePlayableUrl(it.url); m_session.activeTitle=it.title; m_session.activeTimelineHash=it.timelineHash; m_session.requestedPosition=it.position; ApplyPendingSeek(pMainFrame); EmitPlaylistItemChanged(pMainFrame, "prev"); EmitPlaybackStateChanged(pMainFrame); EmitPositionTick(pMainFrame, true);} FillStatus(pMainFrame, body); return true; }
