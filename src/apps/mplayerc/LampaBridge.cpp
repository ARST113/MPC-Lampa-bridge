...
void CLampaBridge::SyncActiveFromPlayer(CMainFrame* pMainFrame, const CStringA& reason)
{
	...
	if (found < 0) {
		CString curName = LampaFileNameOnly(cur);
		for (size_t i = 0; i < m_session.items.size(); ++i) {
...
static CString LampaFileNameOnly(const CString& in)
{
	CString s = in;
	int frag = s.Find(L"#");
	if (frag >= 0) s = s.Left(frag);
	int q = s.Find(L"?");
	if (q >= 0) s = s.Left(q);
	int slash = s.ReverseFind(L'/');
	int bslash = s.ReverseFind(L'\\');
	int pos = slash > bslash ? slash : bslash;
	return pos >= 0 ? s.Mid(pos + 1) : s;
}
...
bool CLampaBridge::OpenIndex(CMainFrame* pMainFrame, int index){ if(!pMainFrame||index<0||index>=(int)m_session.items.size()) return false; std::list<CString> cmdln; for (const auto& item : m_session.items) { CString clean = NormalizePlayableUrl(item.url); if (!clean.IsEmpty()) cmdln.emplace_back(clean);} if (cmdln.empty()) return false; int len=0; for(auto&s:cmdln){len+=(s.GetLength()+1)*sizeof(WCHAR);} std::unique_ptr<BYTE[]> buff(new BYTE[4+len]); BYTE* p=buff.get(); *(DWORD*)p=(DWORD)cmdln.size(); p+=4; for(auto&s:cmdln){int l=(s.GetLength()+1)*sizeof(WCHAR); memcpy(p,(LPCWSTR)s,l); p+=l;} COPYDATASTRUCT cds{0x6ABE51,(DWORD)(p-buff.get()),buff.get()}; pMainFrame->SendMessageW(WM_COPYDATA,0,(LPARAM)&cds); pMainFrame->m_wndPlaylistBar.SetSelIdx(index, true); pMainFrame->OpenCurPlaylistItem(); auto& it=m_session.items[index]; m_session.activeIndex=index; m_session.activeUrl=NormalizePlayableUrl(it.url); m_session.activeTitle=it.title; m_session.activeTimelineHash=it.timelineHash; m_session.requestedPosition=it.position; m_session.timelinePercent=it.timelinePercent; m_session.timelineDuration=it.duration; return true; }
