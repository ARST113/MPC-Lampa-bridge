...
void CLampaBridge::EmitSessionStarted(CMainFrame* pMainFrame){ CStringA p; const CStringA sid = EscA(m_session.sessionId); const CStringA uri = EscA(m_session.activeUrl); const CStringA title = EscA(m_session.activeTitle); p.Format("{\"sessionId\":\"%s\",\"ts\":%lld,\"uri\":\"%s\",\"title\":\"%s\",\"playlistSize\":%d,\"startIndex\":%d,\"startPosition\":%lld,\"currentItem\":{\"uri\":\"%s\",\"title\":\"%s\"}}", sid.GetString(), NowMs(), uri.GetString(), title.GetString(), (int)m_session.items.size(), m_session.activeIndex, PositionMs(pMainFrame), uri.GetString(), title.GetString()); EmitEvent("session_started", p, true); }
...
void CLampaBridge::EmitPlaylistItemChanged(CMainFrame* pMainFrame, const CStringA& reason){ CStringA p; const CStringA uri = EscA(m_session.activeUrl); const CStringA title = EscA(m_session.activeTitle); p.Format("{\"uri\":\"%s\",\"windowIndex\":%d,\"playlistSize\":%d,\"title\":\"%s\",\"reason\":\"%s\",\"position\":%lld,\"duration\":%lld,\"hasPrevious\":%s,\"hasNext\":%s,\"currentItem\":{\"uri\":\"%s\",\"title\":\"%s\"}}", uri.GetString(), m_session.activeIndex, (int)m_session.items.size(), title.GetString(), reason.GetString(), PositionMs(pMainFrame), DurationMs(pMainFrame), m_session.activeIndex>0?"true":"false", m_session.activeIndex+1<(int)m_session.items.size()?"true":"false", uri.GetString(), title.GetString()); EmitEvent("playlist_item_changed", p, true); }
...
bool CLampaBridge::State(const CString& sid, const CString& token, CMainFrame* pMainFrame, CStringA& body){ if (sid != m_session.sessionId || !CheckToken(token)) { body = "{\"ok\":false,\"error\":\"forbidden\"}"; return true; } EmitPositionTick(pMainFrame, false); body = LampaBridgeJson::BuildStateResponse(m_session.store.hasLastState ? &m_session.store.lastState : nullptr); return true; }
...
bool CLampaBridge::OpenIndex(CMainFrame* pMainFrame, int index){ if(!pMainFrame||index<0||index>=(int)m_session.items.size()) return false; std::list<CString> cmdln; for (const auto& item : m_session.items) { CString clean = NormalizePlayableUrl(item.url); if (!clean.IsEmpty()) cmdln.emplace_back(clean);} if (cmdln.empty()) return false; int len=0; for(auto&s:cmdln){len+=(s.GetLength()+1)*sizeof(WCHAR);} std::unique_ptr<BYTE[]> buff(new BYTE[4+len]); BYTE* p=buff.get(); *(DWORD*)p=(DWORD)cmdln.size(); p+=4; for(auto&s:cmdln){int l=(s.GetLength()+1)*sizeof(WCHAR); memcpy(p,(LPCWSTR)s,l); p+=l;} COPYDATASTRUCT cds{0x6ABE51,(DWORD)(p-buff.get()),buff.get()}; pMainFrame->SendMessageW(WM_COPYDATA,0,(LPARAM)&cds); m_session.activeIndex=0; int hops = index; while (hops-- > 0) pMainFrame->SendMessageW(WM_COMMAND, ID_NAVIGATE_SKIPFORWARD); auto& it=m_session.items[index]; m_session.activeIndex=index; m_session.activeUrl=NormalizePlayableUrl(it.url); m_session.activeTitle=it.title; m_session.activeTimelineHash=it.timelineHash; m_session.requestedPosition=it.position; m_session.timelinePercent=it.timelinePercent; m_session.timelineDuration=it.duration; return true; }
...
bool CLampaBridge::Command(CMainFrame* pMainFrame, const CStringA& payloadBody, CStringA& body){ rapidjson::Document d; if(d.Parse(payloadBody).HasParseError()||!d.IsObject()){ body="{\"ok\":false,\"error\":\"invalid json\"}"; return true;} CStringA cmd; getJsonValue(d,"command",cmd); cmd.MakeLower(); if(cmd=="play"){ pMainFrame->SendMessageW(WM_COMMAND,ID_PLAY_PLAY); EmitPlaybackStateChanged(pMainFrame);} else if(cmd=="pause"){ pMainFrame->SendMessageW(WM_COMMAND,ID_PLAY_PAUSE); EmitPlaybackStateChanged(pMainFrame);} else if(cmd=="stop"){ pMainFrame->SendMessageW(WM_COMMAND,ID_PLAY_STOP); EmitPlaybackStateChanged(pMainFrame); EmitSessionFinished(pMainFrame, "user");} else if(cmd=="seek"){ double tm=0; if(d.HasMember("time")&&d["time"].IsNumber()) tm=d["time"].GetDouble(); int64_t from=PositionMs(pMainFrame); pMainFrame->SeekTo((REFERENCE_TIME)(tm*10000000.0)); EmitSeekCompleted(pMainFrame, from, (int64_t)(tm*1000.0)); EmitPositionTick(pMainFrame, true);} else if(cmd=="next"&&m_session.activeIndex+1<(int)m_session.items.size()){ OpenIndex(pMainFrame,m_session.activeIndex+1); ApplyPendingSeek(pMainFrame); EmitPlaylistItemChanged(pMainFrame, "next"); EmitPlaybackStateChanged(pMainFrame); EmitPositionTick(pMainFrame, true);} else if(cmd=="prev"&&m_session.activeIndex>0){ OpenIndex(pMainFrame,m_session.activeIndex-1); ApplyPendingSeek(pMainFrame); EmitPlaylistItemChanged(pMainFrame, "prev"); EmitPlaybackStateChanged(pMainFrame); EmitPositionTick(pMainFrame, true);} FillStatus(pMainFrame, body); return true; }
